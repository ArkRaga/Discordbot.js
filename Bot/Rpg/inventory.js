...
const getinv = (id) => {
  var inv = inventorys.filter((x) => x.discordid == id);
  if (inv.length > 0) {
...
const editinvitem = (id, item, key, value) => {
  var inv = getinv(id);
  var item = getinvitem(inv, item);
  item[key] = value;
};

const getinvitem = (inv, i) => {
  // Get an item in a inventory
  var item = inv.items.filter((x) => x.name == i.name);
  if (item.length > 0) {
...
const itemEmbed = (item) => {
  var emn = require("../embeds");
  var em = emn.itemEmbed;
  em.files = [];
...
const craft = (args, message) => {
  ...
  var inv = getinv(message.author.id);
...
const giveitem = (args, message) => {
  var userinv = getinv(message.author.id);
  var item = getitem(args);
  var i = item ? getinvitem(userinv, item) : false;
  if (!item) {
