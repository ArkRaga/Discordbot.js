...
class Item {
  constructor({ id, name = "", quantity = 1, type = "material" }) {
    this.id = id;
    this.name = name;
...
const createDrop = (n, num) => {
  const itm = itemDictionary[n];
  if (!itm) {
    return;
  }
  // console.log("items-L40: ", itm);
  return new Item({
    id: itm.id,
    name: itm.name,
    quantity: num,
...
const StartUp = async () => {
  await database
    ...
    .then((items) => {
      items.forEach((ele) => {
        // console.log(ele);
        const i = new Item({ id: ele.item_id, name: ele.name, type: ele.type });
        itemDictionary[ele.name.split(" ").join("").toLowerCase()] = i;
      });
    })
    .catch((err) => {
      console.log("err in items start up");
    });
