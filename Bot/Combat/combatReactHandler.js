...
const startcombat = async (args, message) => {
  ...
  if (!en) {
    return await message.channel.send('please pick a fight with an actual user')
  } else {
    if (en.user.id == id) {
      return await message.channel.send('you want to fight yourself?, cmon man....')
    }
  }
  if (combats.length != 0) {
  ...
  if (en.user.id == '792163801072009236') {
    ...
    await message.channel.send('You dare challenge me?! I Accept your foolish request!')
...
const hunt = async (args, message) => {
  ...
  await message.channel.send(`<@!${message.author.id}> your fighting a **${en.name}** `)
...
const doHealthCheck = async (combat, message) => {
  ...
  combat.state = combat.state === combatState[0] ? combatState[1] : combatState[0]
...
const getcombat = async (args, message) => {
  ...
  if (combat.length != 0) {
    ...
    await message.channel.send('You are not in combat currently. go pick a fight.')
  }
}
